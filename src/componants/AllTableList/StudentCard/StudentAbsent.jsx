...
const StudentAbsent = () => {
  const API_URL = import.meta.env.VITE_API_URL; // API host
  const navigate = useNavigate();
  const location = useLocation();
  const query = new URLSearchParams(location.search);
  const tabFromQuery = query.get("tab");
  ...
  // Preloaded third tab data
  const [notMarkData, setNotMarkData] = useState([]);
  const [loadingNotMark, setLoadingNotMark] = useState(false);

  // Replace URL query without reload
  useEffect(() => {
    if (tabFromQuery) {
      window.history.replaceState({}, "", "/studentAbsent");
    }
  ...
  // Fetch class teacher info after regId & roleId are set
  useEffect(() => {
    if (!roleId || !regId) return;
    fetchClassTeacherData();
  }, [roleId, regId]);
