...
const BooksAddedInLibraryReport = () => {
  ...
  const [selectedSourceBook, setSelectedSource] = useState("");
  ...
  const fetchSourceofBook = async () => {
    try {
      const token = localStorage.getItem("authToken");

      // build URL conditionally
      let url = `${API_URL}/api/library/get_source_of_book  `;

      const response = await axios.get(url, {
        headers: { Authorization: `Bearer ${token}` },
      });

      // console.log("all souce book name:", response.data.data);

      if (Array.isArray(response.data.data)) {
        setSourceBook(response.data.data);
      } else {
        setSourceBook([]);
        console.error("Unexpected API response format:", response.data);
      }
  ...
  const handleSourceSelect = (selectedOption, actionMeta) => {
    if (actionMeta.action === "clear") {
      isClearingRef.current = true;
    ...
    setSelectedSourceId(selectedOption.value);
  };

  const sourceOptions = useMemo(() => {
    if (!Array.isArray(sourceBook)) return [];
    return sourceBook.map((cls) => ({
      value: cls?.value,
      label: `${cls?.label}`,
    }));
  }, [sourceBook]);
    ...
    try {
      ...
      if (!clearAll) {
        ...
        if (selectedSourceBook)
          params.source_of_book = selectedSourceBook.value;
        ...
        <div className="card rounded-md ">
          ...
          <>
            <div className="flex flex-wrap gap-4 px-4">
              ...
              {/* Call Category */}
              <div className="flex flex-col w-full sm:w-[48%] md:w-[15%]">
                <label className="text-md mb-1">Call Category</label>
                <Select
              ...
              {/* Source of Book */}
              <div className="flex flex-col w-full sm:w-[48%] md:w-[15%]">
                <label className="text-md mb-1">Source of Book</label>
                <Select
...
export default BooksAddedInLibraryReport;
