...
const BooksIssuedHistoryReport = () => {
  ...
  const [debouncedAccessionNo, setDebouncedAccessionNo] = useState("");
  ...
  useEffect(() => {
    ...
    const fetchBookByCopy = async () => {
      try {
        const token = localStorage.getItem("authToken");

        const res = await axios.post(
          `${API_URL}/api/library/get_book_by_copy`,
          { copy_id: debouncedAccessionNo },
    ...
    fetchBookByCopy();
  }, [debouncedAccessionNo]);

  useEffect(() => {
    if (isClearingRef.current || isAccessionResetRef.current) {
      isClearingRef.current = false;
      isAccessionResetRef.current = false;
      return;
    }

    if (searchFrom || searchTo || debouncedAccessionNo) {
      handleSearch();
    }
  ...
  const handleSearch = async ({ clearAll = false } = {}) => {
    ...
    try {
      ...
      if (!clearAll) {
        if (assessionNo) params.copy_id = assessionNo;
        if (searchFrom) params.from_date = searchFrom;
        if (searchTo) params.to_date = searchTo;
      }

      // console.log("Search Params:", params);

      const response = await axios.get(
        `${API_URL}/api/library/book_issued_history_report`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params,
  ...
  const handlePrint = () => {
    const printTitle = `Book Issued History Report`;
    const printContent = `
  ...
  const handleDownloadEXL = () => {
    ...
    const data = filteredSections.map((student, index) => [
      ...
      student?.member_type === "S" ? "Student" : "Staff",
      camelCase(
        `${student?.first_name || ""} ${student?.mid_name || ""} ${student?.last_name || ""}`.trim(),
      ),
    ]);
    ...
    const fileName = `Book Issued History Report.xlsx`;
        ...
        <div className="card rounded-md ">
          ...
          <>
            <div className="flex flex-wrap gap-4 px-4">
              {/* Category Group */}
              <div className="flex flex-col w-full sm:w-[48%] md:w-[15%]">
              ...
              <div className="flex flex-col w-full sm:w-[48%] md:w-[30%]">
                ...
                <input
                  ...
                  disabled
                  readOnly
                  className="border border-gray-300 bg-gray-100 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                />
              </div>

              {/* Date */}
              <div className="flex flex-col w-full sm:w-[48%] md:w-[30%] py-0">
                <label className="text-md mb-1">Date</label>
                              ...
                              <div className="absolute bottom-2 left-4 z-10">
                                <div className="px-4 py-2 bg-gray-100 border rounded shadow text-sm">
                                  <span className="text-blue-800 font-semibold">
                                    Total Books Issued for Accession No.{" "}
                                    {assessionNo} are :
                                  </span>
...
export default BooksIssuedHistoryReport;
