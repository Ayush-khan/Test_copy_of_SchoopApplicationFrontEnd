...
import AllotHPCSubjectTab from "./AllotHPCSubjectTab";
import Select from "react-select";
function HPCSubjectAllotment() {
  ...
  //     const name = cls?.name?.toString().trim().toLowerCase();

  //     // Always include Nursery, LKG, UKG
  //     if (["nursery", "lkg", "ukg"].includes(name)) return true;

  //     // For numbered classes, include only if <= 2
  ...
  const handleSearch = async () => {
    ...
    setSearchTerm("");
    try {
      console.log("class selected", classIdForManage);
      const token = localStorage.getItem("authToken");
      const response = await axios.get(
        `${API_URL}/api/get_hpc_subject_Alloted_for_report_card/${classIdForManage}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (response?.data?.subjectAllotments.length > 0) {
        ...
        setSubjects([]);
        toast.error("No HPC subjects found for the selected class.");
      }
    } catch (error) {
  ...
  const handleDelete = (sectionId) => {
    const classToDelete = subjects.find(
      (cls) => cls.hpc_subject_id === sectionId
    );

    // Set the current section and subject name for deletion
    if (classToDelete) {
      setCurrentSection(classToDelete); // Set the current section directly
      setCurrestSubjectNameForDelete(classToDelete?.subject_name); // Set subject name for display
  ...
  const handleSubmitDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const subReportCardId = currentSection?.hpc_subject_id; // Get the correct ID
      ...
      const response = await axios.delete(
        `${API_URL}/api/delete_hpc_sub_report_allotted/${subReportCardId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const filteredSections = subjects.filter((section) => {
    ...
    const subjectName = section?.subject_name?.toLowerCase() || "";
    ...
    <>
      ...
      <div className="md:mx-auto md:w-3/4 p-4 bg-white mt-4 ">
        <h3 className="text-gray-700 mt-1 text-[1.2em] lg:text-xl text-nowrap">
          Subject Allotment For HPC Report Card
        </h3>
        <div
        ...
        ></div>
        <ul className="grid grid-cols-2 gap-x-10  -left-6 md:left-0 md:flex md:flex-row relative -top-4">
          {/* Tab Navigation */}
                              ...
                              displayedSections.map((subject, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-2 text-center lg:px-3 py-2 border border-gray-950 text-sm">
                                    <button
                                      onClick={() =>
                                        handleDelete(subject?.hpc_subject_id)
                                      }
...
export default HPCSubjectAllotment;
