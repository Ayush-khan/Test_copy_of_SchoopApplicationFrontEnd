...
const EditDomainDetails = () => {
  ...
  const [domainParameter, setDomainParameter] = useState(null);
  const hasPrefilled = useRef(false);
  const [subjectOptions, setSubjectOptions] = useState([]);

  useEffect(() => {
    fetchDomainDetailData();
    fetchClassNames();
    fetchCompentencies();
  }, []);
  ...
  const fetchDomainDetailData = async () => {
    try {
      const token = localStorage.getItem("authToken");
      const response = await axios.get(
        `${API_URL}/api/edit_domainparameters/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      console.log("domain data", response.data.data.domain);
      console.log("paramtere dataa", response.data.data.parameters);
      setDomainData(response.data.data.domain || []);
      setDomainParameter(response.data.data.parameters || []);
    } catch (error) {
      toast.error("Error fetching domain");
    }
  };
  ...
  const handleSubmitUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/api/update_domainparameters/${id}`,
        payload,
        {
          headers: {
