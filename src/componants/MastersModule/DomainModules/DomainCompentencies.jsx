...
function DomainCompentencies() {
  ...
  const handleSubmitAdd = async () => {
    ...
    console.log("new compentencies", newLeaveType);
    ...
    try {
      ...
      const checkleaveType = await axios.post(
        `${API_URL}/api/save_domaincompetencies`,
        { name: newLeaveType },
        {
          headers: { Authorization: `Bearer ${token}` },
      ...
      setNameError("");
      toast.success("Compentency added successfully!");
      setNameAvailable(true);

      fetchSections(); // Fetch updated data
      handleCloseModal(); // Close the modal
    } catch (error) {
      console.error("Error adding Compentency:", error);
  ...
  const handleSubmitEdit = async () => {
    ...
    try {
      ...
      if (!token || !currentSection) {
        throw new Error("No authentication token or compentancy ID found.");
      }
      console.log("Current Section:", currentSection);

      const response = await axios.put(
        `${API_URL}/api/update_domaincompetencies/${currentSection.dm_competency_id}`,
        { name: newLeaveType },
        {
          headers: { Authorization: `Bearer ${token}` },
          withCredentials: true,
      ...
      if (response.data.success === false) {
        toast.error("Compentency name already exists");
      } else {
        toast.success("Compentency updated successfully!");
      }
  ...
  const handleDelete = (id) => {
    ...
    const sectionToDelete = sections.find(
      (leave) => leave.dm_competency_id === id
    );
  ...
  const handleSubmitDelete = async () => {
    ...
    try {
      ...
      if (!token || !currentSection || !currentSection.dm_competency_id) {
        throw new Error("Compentency is missing");
      }

      const response = await axios.delete(
        `${API_URL}/api/delete_domaincompetencies/${currentSection.dm_competency_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.data.success) {
        setShowDeleteModal(false);
        setCurrentSection(null);
        toast.success("Compentency deleted successfully!");
      } else {
        toast.error(response.data.message || "Failed to delete Compentency ");
      }
    } catch (error) {
      console.error("Error deleting Compentency:", error);
                          ...
                          {roleId === "M" ? (
                            ...
                            <td className="text-center px-2 lg:px-3 border border-gray-950 text-sm">
                              <button
                                ...
                                onClick={() =>
                                  handleDelete(leave.dm_competency_id)
                                }
                              >
              ...
              >
                <div className="modal-content">
                  <div className="flex justify-between p-3">
                    <h5 className="modal-title">Create Compentencies</h5>
        ...
        {showEditModal && (
          ...
          >
            ...
            >
              <div className="modal-content">
                <div className="flex justify-between p-3">
                  <h5 className="modal-title">Edit Compentencies</h5>
                  <RxCross1
          ...
          >
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <p>
                    Are you sure you want to delete compentencies:{" "}
                    {currentSection.name}?
                  </p>
                </div>
                <div className=" flex justify-end p-3">
...
export default DomainCompentencies;
