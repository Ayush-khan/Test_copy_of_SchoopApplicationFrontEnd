...
//   const storedSearchData = useRef(null);
//   // State for form fields and validation errors
//   const [setPassword, setSetpassword] = useState("");
//   const [userIdset, setUserIdset] = useState("");
//   const [passwordError, setPasswordError] = useState(""); // For password error
...
//       if (storedSearchData.current && !hasRestoredSearch.current) {
//         const { classIdForManage, selectedStudentId, grNumber } =
//           storedSearchData.current;

//         const restoreSearchState = async () => {
//           const token = localStorage.getItem("authToken");

//           // Step 1: Restore Class & fetch students if needed
//           if (classIdForManage) {
//             const foundClass = classResponse.data.find(
//               (cls) => cls.section_id === classIdForManage
//             );

//             if (foundClass) {
//               setSelectedClass({
//                 value: foundClass.section_id,
//                 label: `${foundClass?.get_class?.name} ${foundClass.name} (${foundClass.students_count})`,
//               });
//               setclassIdForManage(classIdForManage);
//               setSectionIdForStudentList(classIdForManage);
...
//               const studentList = updatedStudents?.data?.data || [];
//               setStudentNameWithClassId(studentList);

//               // Step 2: Restore student
//               if (selectedStudentId) {
//                 const foundStudent = studentList.find(
//                   (stu) => stu.student_id === selectedStudentId
//                 );

//                 if (foundStudent) {
//                   const studentLabel = [
//                     foundStudent.first_name,
//                     foundStudent.mid_name,
...
//           // Step 3: Restore GR Number
//           if (grNumber) setGrNumber(grNumber);

//           // Step 4: Wait a bit to ensure states are fully updated, then call search
//           setTimeout(() => {
//             // Only call search if at least one is present
//             const hasClass = !!classIdForManage;
//             const hasStudent = !!selectedStudentId;
//             const hasGR = !!grNumber;

//             if (hasClass || hasStudent || hasGR) {
//               handleSearch(); // One-time call
//             }

//             hasRestoredSearch.current = true;
//           }, 300); // Delay gives React state time to update
//         };
...
//   useEffect(() => {
//     const savedSearch = localStorage.getItem("studentSearchState");
//     if (savedSearch) {
//       storedSearchData.current = JSON.parse(savedSearch);
//     }
//   }, []);

//   useEffect(() => {
//     const savedSearch = localStorage.getItem("studentSearchState");
//     if (!savedSearch || hasRestoredSearch.current) return;

//     const { selectedStudentId, classIdForManage, grNumber } =
//       JSON.parse(savedSearch);

//     // Only run restore when dropdown options are ready
//     const classReady = classes.length > 0;
//     const studentReady = studentNameWithClassId.length > 0;

//     if (!classReady || !studentReady) return;
...
//     if (selectedStudentId) {
//       const foundStudent = studentNameWithClassId.find(
//         (stu) => stu.student_id === selectedStudentId
//       );
//       if (foundStudent) {
//         const studentLabel = [
...
//     if (grNumber) {
//       setGrNumber(grNumber);
//     }

//     // Delay to ensure dropdowns are rendered before searching
//     setTimeout(() => {
//       handleSearch();
//     }, 200);

//     hasRestoredSearch.current = true;
//   }, [classes, studentNameWithClassId]);
...
//     saveSearchToLocalStorage(); // ✅ Save state
//     navigate(
//       `/student/edit/${section.student_id}`,
