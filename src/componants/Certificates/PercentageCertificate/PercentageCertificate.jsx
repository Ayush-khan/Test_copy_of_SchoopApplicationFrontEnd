...
function PercentageCertificate() {
  ...
  //       `${API_URL}/api/get_percentagebonafidecertificatelist`,
  //       {
  //         headers: { Authorization: `Bearer ${token}` },
  ...
  //       setSubjects([]);
  //       toast.error(
  //         "No Percentage certificates Listing are found for the selected class."
  //       );
  //     }
  //   } catch (error) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      console.log("Percentage Certificate List Response:", response.data);
      ...
      if (response?.data?.data?.length > 0) {
        ...
        setSubjects([]);
        toast.error("No Percentage certificates found for the selected class.");
      }
    } catch (error) {
      console.error("Error fetching Percentage certificates:", error);
      setError("Error fetching Percentage certificates");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleEditForm = (section) => {
    ...
    navigate(`/stud_Percent/edit/${section?.sr_no}`, {
      state: {
        student: section,
  ...
  const handleDownloadSumbit = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/get_percentageisDownload/${currentSection.sr_no}`,
        {
          headers: { Authorization: `Bearer ${token}` },
      ...
      if (error.response && error.response.data) {
        toast.error(
          `Error in Downloading Percentage Certificate: ${error.response.data.error}`
        );
      } else {
        toast.error(
          `Error in Downloading Percentage Certificate: ${error.message}`
        );
      }
  ...
  const handleSubmitEdit = async () => {
    ...
    try {
      ...
      await axios.put(
        `${API_URL}/api/update_percentageisIssued/${currentSection.sr_no}`,
  ...
  const handleSubmitDelete = async () => {
    ...
    try {
      ...
      await axios.delete(
        `${API_URL}/api/delete_percentageisDeleted/${subReportCardId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    <>
      ...
      <div className="md:mx-auto md:w-3/4 p-4 bg-white mt-4 ">
        ...
        <ul className="grid grid-cols-2 gap-x-10 relative -left-6 md:left-0 md:flex md:flex-row relative -top-4">
          {/* Tab Navigation */}
          {["Manage", "CreatePercentageCertificate"].map((tab) => (
            <li
              key={tab}
        ...
        <div className="bg-white  rounded-md -mt-5">
          ...
          {activeTab === "CreatePercentageCertificate" && (
            <div>
              <CreatePercentageCertificate />
            </div>
          )}
        </div>
      </div>
...
export default PercentageCertificate;
