    ...
    const fetchInitialData = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_URL}/api/get_characterdata/${student?.sr_no}`,
          {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/api/update_charactercertificate/${student?.sr_no}`,
        formattedFormData,
        {
          headers: {
      ...
      if (response.status === 200) {
        toast.success("Character Certificate updated successfully!");
