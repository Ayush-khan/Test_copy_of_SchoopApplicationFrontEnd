...
const IciciFeePaymentReport = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/geticicifeepaymentreport`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params,
  ...
  const handlePrint = () => {
    const printTitle = `ICICI Fee Payment Report ${
        ...
        <thead>
          <tr class="bg-gray-100">
            ...
            <th class="px-2 text-center py-2 border border-black text-sm font-semibold">Receipt No.</th>
             <th class="px-2 text-center py-2 border border-black text-sm font-semibold">Transcation ID </th>
          ...
          ${displayedSections
            .map(
              ...
              <tr class="text-sm">
                ...
                <td class="px-2 text-center py-2 border border-black">${
                  formatDateToShort(subject?.Trnx_date) || ""
                }</td>
                 <td class="px-2 text-center py-2 border border-black">${
  ...
  const handleDownloadEXL = () => {
    ...
    const data = displayedSections.map((student, index) => [
      ...
      student?.class_name || " ",
      `${formatDateToShort(student?.Trnx_date) || ""}`,
    ...
    const fileName = `ICICI_Fee_Payment_Report.xlsx`;
  ...
  const filteredSections = timetable.filter((student) => {
    ...
    const dateofTrnx = normalize(formatDateToShort(student?.Trnx_date));
    const installmentNo = normalize(student?.installment_no);
    const trnxRefNo = normalize(student?.txnID);
...
export default IciciFeePaymentReport;
