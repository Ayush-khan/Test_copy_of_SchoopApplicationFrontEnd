...
const AttendanceDetaileMontReport = () => {
  ...
  const handleMonthSelect = (selectedOption) => {
    ...
    setSelectedMonth(selectedOption);
    setSelectedMonthId(selectedOption?.value); // This is the final payload value
  ...
  const monthOptions = [
    ...
    value: `${month.value}-${academicYear || new Date().getFullYear()}`, // fallback if academicYear is undefined
    label: month.label,
  }));
      ...
      ${students
        ...
        .map(
          ...
          <tr style="${zebraStyle(i)}">
            <td style="border: 1px solid #ccc; padding: 6px;">${student.rollNo
            }</td>
            ...
            ${student.attendance
              .map(
                (val) => `
                <td style="border: 1px solid #ccc; padding: 6px; ${val.status === "A" ? "color: #dc2626; font-weight: bold;" : ""
                  }">
  ...
  `;
    ...
    <tfoot style="background-color: #fefce8;">
      <tr style="color: #16a34a; font-weight: 600;">
        ...
        .join("")}
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_present_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_prev_attendance
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_attendance
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
      ...
      <tr style="color: #dc2626; font-weight: 600;">
        ...
        .join("")}
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_absent_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_absent_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_prev_absent_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.grand_total_absent_attendance
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
        <td style="border: 1px solid #ccc; padding: 6px;">–</td>
      ...
      <tr style="color: #1e3a8a; font-weight: bold;">
        ...
        .join("")}
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_present_absent_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_absent_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_previous_attendance
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.grand_total_attendance
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_cumulative_absent_days
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_working_days_for_this_month
      }</td>
        <td style="border: 1px solid #ccc; padding: 6px;">${timetable.totals.total_working_days_till_month
      }</td>
      </tr>
    </tfoot>
  `;
  ...
  const handlePrint = () => {
    ...
    const headerTable = `
      <table style="width: 100%; margin-bottom: 10px;  font-size: 14px;">
        <tr>
          <td style="border: 1px solid #ccc; padding: 6px; text-align: center;"><strong>Class:</strong> ${selectedStudent?.class || ""
      }</td>
          <td style="border: 1px solid #ccc; padding: 6px; text-align: center;"><strong>Division:</strong> ${selectedStudent?.section || ""
      }</td>
          <td style="border: 1px solid #ccc; padding: 6px; text-align: center;"><strong>Month:</strong> ${selectedMonth?.label
      }</td>
        </tr>
      </table>
    `;
...
date.</p>
  ...
  return (
    <>
      <div
        className={` transition-all duration-500 w-[85%]  mx-auto p-4 ${showStudentReport ? "w-full " : "w-[85%] "
          }`}
        ...
        <div className="card pb-4  rounded-md ">
          {!showStudentReport && (
            <>
              ...
              <div
                className={` relative    -top-6 h-1  mx-auto bg-red-700 ${showStudentReport ? "w-full " : "w-[98%] "
                  }`}
                style={{
                                  ...
                                  filteredStudents.map((student, i) => (
                                    ...
                                    >
                                      ...
                                      {student.attendance.map((val, idx) => (
                                        <td
                                          key={idx}
                                          className={`border p-2 ${val.status === "A"
