...
const WorldlineFeePaymentReport = () => {
  ...
  //     //   setAccountError("Please select Acoount type.");
  //     //   setLoadingForSearch(false);
  //     //   return;
  //     // }
  ...
  //       const token = localStorage.getItem("authToken");
  //       const params = {};
  //       if (selectedAccountId) params.account_type = selectedAccountId;
  ...
  //       //   toast.error("Error fetching Consolidated Leave Report. Please try again.");
  //     } finally {
  ...
  //         return;
  //     }
  //     setAccountError(""); // Clear previous errors
  //     setFormDateError(""); // Clear previous errors
  //     setToDateError("");
  //     setSearchTerm("");
  ...
  //         const params = {
  //             account_type: selectedAccount.label, // ✅ Sending the name instead of the value
  //             from_date: fromDate,
  //             to_date: toDate
  //         };
  //         if (selectedStudentId) params.student_id = selectedStudentId;
  ...
  const handlePrint = () => {
    const printTitle = `Worldline Fee Payment Report ${
  ...
  //   const orderId = student?.OrderId?.trim().toLowerCase() || "";
  ...
  //       .replace(/\s+/g, " ")
  //       .trim() || "";
  //   const trnxRefNo = student?.Trnx_ref_no?.toString().toLowerCase() || "";
  ...
  //     trnxRefNo.includes(searchLower) ||
  ...
  const filteredSections = timetable.filter((student) => {
    ...
    const formatDate = (dateString) => {
      ...
      // Ensure we remove the time part if present
      const cleanDate = dateString.split(" ")[0]; // Remove time part if present
      const [year, month, day] = cleanDate.split("-");
      return `${day}/${month}/${year} || ${day}-${month}-${year}`;
    };
    ...
    const trnxRefNo = normalize(student?.Trnx_ref_no);
                    ...
                    <div className="p-2 px-3 bg-gray-100 border-none flex justify-between items-center">
                      <div className="w-full flex flex-row justify-between mr-0 md:mr-4 ">
                        <h3 className="text-gray-700 mt-1 text-[1.2em] lg:text-xl text-nowrap">
                          List of Worldline Fee Payment Report
                        </h3>
