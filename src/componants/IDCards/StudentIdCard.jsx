...
import testImage from "../../assets/HPC/SACS/HPC_Cover/Domains.jpg";
...
const StudentIdCard = () => {
  ...
  const handleDownloadEXL = async () => {
    ...
    worksheet.columns = [
      ...
      { header: "Father Mobile No.", key: "f_mobile", width: 15 },
      { header: "Mother Mobile No.", key: "m_mobile", width: 15 },
      { header: "Address", key: "address", width: 25 },
      { header: "Blood Group", key: "blood", width: 12 },
      { header: "Grn No.", key: "grn", width: 12 },
      { header: "House", key: "house", width: 12 },
      { header: "Image Name", key: "image_name", width: 20 },
    ];

    // Convert local image to buffer ONCE
    ...
    for (let i = 0; i < displayedSections.length; i++) {
      const subject = displayedSections[i];

      worksheet.addRow({
        sr: i + 1,
        roll: subject?.roll_no || "",
        class: `${subject?.class_name || ""} ${subject?.sec_name || ""}`,
        name: `${subject?.first_name || ""} ${subject?.mid_name || ""} ${subject?.last_name || ""}`,
        dob: subject?.dob
          ? new Date(subject.dob).toLocaleDateString("en-GB")
          : "",
        f_mobile: subject?.f_mobile || "",
        m_mobile: subject?.m_mobile || "",
        address: subject?.permant_add || "",
        blood: subject?.blood_group || "",
        grn: subject?.reg_no || "",
        house: subject?.house === "Unknown House" ? "" : subject?.house,
        image_name: subject?.image_name || "",
      });

      // STEP 1: Build correct full URL
      const imageUrl = subject?.image_url;
      ...
      if (imageUrl) {
        try {
          ...
          // STEP 4: detect type
          const extension = imageUrl.toLowerCase().includes(".png")
            ? "png"
            : "jpeg";
          ...
          worksheet.addImage(imageId, {
            tl: { col: 2, row: i + 1 }, // Column C
            ext: { width: 60, height: 60 },
          });

          worksheet.getRow(i + 2).height = 50;
        } catch (error) {
