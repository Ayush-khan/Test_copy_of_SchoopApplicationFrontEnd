...
const StudentIdCard = () => {
  ...
  useEffect(() => {
    if (sectionID) {
      setSelectedStudentId(sectionID); // Pre-select the class dropdown
      handleSearch(sectionID); // Automatically fetch data
    }
  }, [sectionID]);
  ...
  //   if (!selectedStudent && !selectedStudentId && !sectionID) {
  //     setStudentError("Please select Class.");
  //     valid = false;
  //   } else {
  //     setStudentError(""); // Clear the error message if valid
  //   }
  ...
  const handleSearch = async () => {
    let valid = true;
    const idToUse = sectionID || selectedStudentId;

    if (!sectionID && !selectedStudentId) {
      setStudentError("Please select Class.");
      valid = false;
    } else {
      setStudentError("");
