...
//       const response = await axios.get(
//         `${API_URL}/api/get_edittimetablebyclasssection/${class_id}/${section_id}`,
//         {
//           headers: {
...
// export default ViewTimeTablePlanner;
...
import { RxCross1 } from "react-icons/rx";
import "react-datepicker/dist/react-datepicker.css";
import ViewCommonComponent from "./ViewCommonComponent";
...
const ViewTimeTablePlanner = () => {
  ...
  const fetchTimetableData = async (teacher_id, class_id, section_id) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/get_timetableviewbyteacher/${class_id}/${section_id}/${staff?.teacher_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
