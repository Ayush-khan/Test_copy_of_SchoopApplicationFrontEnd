...
const ClassWisePeriodAllotment = () => {
  ...
  const handleSearch = async () => {
    setSearchTerm("");
    setLoadingForSearch(true); // <-- move this up
    try {
      setTimetable([]); // Clear old data
      ...
      setIsSubmitting(false);
      setLoadingForSearch(false); // Correctly ends loading
    }
  };
  ...
  const handleSubmitEdit = async () => {
    ...
    try {
      ...
      console.log("Update response:", response.data);
      console.log("fetch classwise data", timetable);

      handleCloseModal();
      await handleSearch();
      console.log("after handlesearch", timetable); // ← this will show old value
