...
const PullGradesData = () => {
  ...
  const handlePull = async () => {
    ...
    try {
      ...
      const params = {
        current_acd_yr: String(currentAcademicYear),
      };

      const response = await axios.post(
        `${API_URL}/api/grades/pull_prev_year`,
        params,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      if (status === true) {
        toast.success(message || "Grades Data pulled successfully.");
        ...
        toast.error(message || "Data already present. Data cannot be pulled!");
        setTimetable([]);
      }
    } catch (error) {
      ...
      if (error?.response?.status === 400) {
        toast.error(
          error?.response?.data?.message ||
            "Data already present. Data cannot be pulled!"
        );
      } else {
        ...
        <div className="card  rounded-md ">
          <div className=" card-header mb-4 flex justify-between items-center ">
            <h5 className="text-gray-700 mt-1 text-md lg:text-lg">
              Pull Grades From Previous Year
            </h5>
            <RxCross1
          ...
          <>
            <div
              className={`w-full mx-auto p-4 ${
                timetable.length > 0 ? "md:w-[50%]" : "md:w-[50%]"
              }`}
            >
