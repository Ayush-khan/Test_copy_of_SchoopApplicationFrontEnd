...
const CreeateCastCertificate = () => {
  ...
  // Fetch classes and students on initial load
  // Fetch classes and students on initial load
  useEffect(() => {
    fetchClasses();
    fetchStudentList(); // Initial call without section_id to fetch all students
  }, []);
  ...
  // Fetch students with or without a section_id
  const fetchStudentList = useCallback(
    debounce(async (section_id = null) => {
      ...
      try {
        ...
        const response = await axios.get(
          ...
          {
            headers: { Authorization: `Bearer ${token}` },
            params: section_id ? { section_id } : {}, // Pass section_id if available
          }
        );
        const studentData = response?.data?.students || [];
        setStudents(studentData);
        setFilteredStudents(studentData); // Set filtered students as either all or section-specific
      } catch (error) {
  ...
  const handleClassSelect = (selectedOption) => {
    ...
    fetchStudentList(selectedOption.value); // Fetch students for the selected class
  };
