...
function Form() {
  ...
  const handleSubmit = async (event) => {
    ...
    // Call the API to check username uniqueness before submitting
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/check-user-id/${student.student_id}/${selectedUsername}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      if (usernameExists) {
        toast.warning(
          "The selected username already exists. Proceeding with caution."
        );
        console.log(
          "Warning: Username already exists. Proceeding with submission."
        );
      }
    } catch (error) {
      console.error("Error while checking username uniqueness:", error);
      toast.warning(
        "Couldn't verify username uniqueness. Proceeding with submission."
      );
    }
