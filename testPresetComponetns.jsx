...
// export default ManageSubjectList;
  //  This is the working component with l ogic {monday work}
  import { useState, useEffect, useRef } from "react";
  import { IoSettingsSharp } from "react-icons/io5";
  import axios from "axios";
  import { ToastContainer, toast } from "react-toastify";
  ...
  function ManageSubjectList() {
    ...
    const uniqueDivisions = Array.from(
      new Map(
        allotSubjectTabData.map((item) => [
          item?.get_division?.section_id,
          {
            id: item?.get_division?.section_id,
            name: item?.get_division?.name,
          },
        ])
      ).values()
    );
    console.log("Unique divisions with IDs:", uniqueDivisions);
    ...
    const uniqueSubjects = Array.from(
      new Map(
        allotSubjectTabData
          ...
          .map((subject) => [
            subject?.subject_id,
            { id: subject?.subject_id, name: subject?.get_subject?.name },
          ])
      ).values()
    );
    console.log("Unique subjects with IDs:", uniqueSubjects);
    // Handle division checkbox change
    const handleDivisionChange = (event) => {
      const id = event.target.value;
      setSelectedDivisions((prevSelected) =>
        prevSelected.includes(id)
          ? prevSelected.filter((divisionId) => divisionId !== id)
          : [...prevSelected, id]
      );
    };
